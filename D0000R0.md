...
> [7]{.pnum} [...] In these cases, the array bound is calculated from the [number of initial elements]{.rm}[initializer:]{.add}  

:::add
> -    [7.1]{.pnum} if the initializer expression is of type "array of `N T`", or
> -    [7.2]{.pnum} the initializer list has one element of type "array of `N T`", or, if not that,
> -    [7.3]{.pnum} the initializer list has `N` elements,

> then the type of the array is "array of `N U`.
:::
